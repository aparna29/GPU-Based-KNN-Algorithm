...
#include <string.h>
#include <time.h>
__global__ void kernel_distance(double* dfeature_arr,int d_numfeatures,int ndim, int d_query,double* d_dist, double* d_label,int k)
{
	int id = blockIdx.x*blockDim.x +  threadIdx.x;
	int tid = threadIdx.x;
	__shared__ double d_arr[258];
	__shared__ int check[258];
	__shared__ double query_obj[110]; // 50 -> num_features
	double d1, d2;
	check[tid] = 1;
	int rank,j;
	...
	for(i = start, j= 0;i<end&&j<ndim;i++, j++)
	{
		d1 = dfeature_arr[i];
		d2 = query_obj[j];
		dis += (d1 - d2)*(d1 - d2);
...
main()
{
    FILE* stream = fopen("residents.csv", "r");
    ...
    int num_blocks = ceil((float)train/num_threads);
     for(ndim=2;ndim<=num_features;ndim+=5){
    cudaEventRecord(st);
    kernel_distance<<<num_blocks,num_threads>>>(dfeature_arr,num_features,ndim,query,d_dist,d_label,k);
    ...
    printf("\n %d\t%lf",ndim,((double) (e - s))* 1000.0 / CLOCKS_PER_SEC +  (double)milliseconds);}
}
